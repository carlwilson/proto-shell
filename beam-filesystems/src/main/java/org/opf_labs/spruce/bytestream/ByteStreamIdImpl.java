 ...
 */
public final class ByteStreamIdImpl implements ByteStreamId, Comparable<ByteStreamId> {
	...
	static final ByteStreamIdImpl NULL_STREAM_ID = new ByteStreamIdImpl(
			0L, ByteStreams.NULL_SHA256.toLowerCase(),
			ByteStreams.NULL_MD5.toLowerCase());
	...
	private ByteStreamIdImpl() {
		this(ByteStreams.UNKNOWN_LENGTH, ByteStreams.NULL_SHA256.toLowerCase(), ByteStreams.NULL_MD5
				.toLowerCase());
	}

	private ByteStreamIdImpl(final long length, final String sha256,
			final String md5) {
		this.length = length;
	...
	static final ByteStreamIdImpl fromValues(final long length,
		...
		if (!ByteStreams.isHexSHA256(sha256))
			throw new IllegalArgumentException(
					"ByteStreams.isHexSHA256(sha256) != true, regex used: "
							+ ByteStreams.HEX_SHA256_REGEX);
		if (!ByteStreams.isHexMD5(md5))
			throw new IllegalArgumentException(
					"ByteStreams.isHexMD5(md5) != true, regex used: "
							+ ByteStreams.HEX_MD5_REGEX);
		if ((sha256 == null) || (sha256.isEmpty()))
			throw new IllegalArgumentException(
					"((sha256 == null) || (sha256.isEmpty())) == true");
		if ((md5 == null) || (md5.isEmpty()))
			throw new IllegalArgumentException(
					"((md5 == null) || (md5.isEmpty())) == true");
		...
		// hex digest strings
		// are equivalent.
		return new ByteStreamIdImpl(length, sha256.toLowerCase(),
				md5.toLowerCase());
	}
	...
	public String toString() {
		return "{\"ByteStreamId\":{\"hexSHA256\":\"" + this.hexSHA256 + "\",\"hexMD5\":\""
	...
	public final int compareTo(final ByteStreamId other) {
		...
		// Sort by 256 hash
		int sha256Diff = this.hexSHA256.compareToIgnoreCase(other
				.getHexSHA256());
		if (sha256Diff != 0)
			return sha256Diff;
		// Sort then by MD5 hash shouldn't have same MD5 and different SHAs
		return this.hexMD5.compareToIgnoreCase(other.getHexMD5());
	}
}
