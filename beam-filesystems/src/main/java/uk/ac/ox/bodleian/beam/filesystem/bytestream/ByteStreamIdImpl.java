 ...
 */
class ByteStreamIdImpl implements ByteStreamId {
	private final String hexSHA256;
	private final String hexMD5;
	private final long length;

	ByteStreamIdImpl() {
		this(0L, ByteStreams.NULL_SHA256, ByteStreams.NULL_MD5);
	}

	ByteStreamIdImpl(final long length, final String sha256, final String md5) {
		if (length < 0L)
		...
		// are equivalent.
		this.length = length;
		this.hexSHA256 = sha256.toLowerCase();
		this.hexMD5 = md5.toLowerCase();
	}

	static final ByteStreamId fromValues(final long length,
			final String sha256, final String md5) {
		return new ByteStreamIdImpl(length, sha256, md5);
	}
